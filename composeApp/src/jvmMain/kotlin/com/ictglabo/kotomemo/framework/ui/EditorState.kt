 ...
 * + applyText) - the state-based modern API in Compose Multiplatform 1.10
 * broke desktop IME composition (Japanese conversion, etc.), so we live
 * on the legacy path until the new API's desktop IME story matures
 * upstream.
 */
class TabState(initial: Contents) {
    ...
    /**
     * Swap the underlying Contents. Save reuses this with identical text
     * just to refresh isDirty / path metadata - preserve the caret in
     * that case so BasicTextField's internal scroll-to-caret does not
     * snap the view to the top after every Ctrl+S.
     */
...
class EditorState(
    ...
    private fun insertAfterSelection(selStart: Int, selEnd: Int, response: String) {
        val tab = current ?: return
        // HTTP servers default to CRLF. Normalise to LF so the buffer
        // invariant (logical LF only) holds.
        ...
        // Restore the original selection (offsets unaffected because insertion is to the right of selEnd)
        tab.applyText(TextFieldValue(newText, TextRange(selStart, selEnd)))
    }
